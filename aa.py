...
def get_encodings(dna, aa):
    rna = dna_to_rna(dna)
    rna_rev_comp = dna_to_rna(motifs.reverse_complement_strand(dna))
    aa_len = 3 * len(aa)
    encodings = [rna_to_dna(kmer) for kmer in motifs.get_all_kmers(rna, aa_len, True) if translate(kmer) == aa]
    encodings_rev = [motifs.reverse_complement_strand(rna_to_dna(kmer)) for kmer in
...
def leaderboard_cyclopeptide_sequencing(spectrum, N=50, integers=False, convolute=False):
    ...
    peptides = itertools.chain.from_iterable(masses_to_peptide(pep) for pep in leader_peptides)
    return set(peptides), leader_score
...
if __name__ == "__main__":
    ...
    best_25 = leaderboard_cyclopeptide_sequencing(spec_25, N, True, convolute=20)
    print(mass_string(best_25[0]))
    # print(len(best_25[0]))
